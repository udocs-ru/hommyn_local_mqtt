...
def open_mqtt_payload(
    host: str, port: int, username: str = "", password: str = "", secure: bool = False
) -> bytes:
    host_b = host.encode()
...
class MdnsCollector(ServiceListener):
    ...
    def add_service(self, zc: Zeroconf, type_: str, name: str) -> None:
        ...
        self.devices[name] = MdnsDevice(
            name,
            info.server.rstrip(".") if info.server else name,
            info.port,
            addresses,
            txt,
        )
