...
def ping_gateway(gateway: str, timeout: float) -> bool:
    ...
    out = run_text_command(
        ["ping", "-c", "1", "-W", str(deadline), gateway], timeout=timeout + 1
    )
...
def get_hotspot_token(
    gateway: str, port: int, timeout: float, retries: int
) -> dict[str, Any]:
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as sock:
        ...
        for attempt in range(1, retries + 1):
            ...
            try:
                ...
                print(
                    "Нет ответа на UDP handshake. Устройство может быть не в режиме сопряжения или порт недоступен."
                )
                time.sleep(1)
                continue
            except OSError as exc:
                raise SystemExit(
                    f"Ошибка UDP при обращении к {gateway}:{port}: {exc}"
                ) from exc
            ...
            time.sleep(1)
    raise SystemExit(
        f"Не удалось получить ненулевой токен. Последний ответ: {last_handshake}"
    )
...
def secure_handshake_probe(device: Any, token_hex: str, timeout: float) -> bool:
    ...
    encrypted_token = aes_cbc_encrypt(
        keys.tx_key_base, keys.tx_iv_base, token, pad=False
    )
...
def discover_target_device(
    ...
    for attempt in range(1, attempts + 1):
        ...
        if wanted_mac:
            matches = [
                device for device in devices if normalize_mac(device.mac) == wanted_mac
            ]
            if matches:
                print(f"Устройство найдено по точному MAC: {wanted_mac}")
                return matches[0]
            near_matches = [
                device for device in devices if is_neighbor_mac(device.mac, wanted_mac)
            ]
            if len(near_matches) == 1:
                print(
                    f"Устройство найдено по соседнему MAC: задан {wanted_mac}, mDNS {near_matches[0].mac}"
                )
        ...
        if hotspot_mac:
            matches = [
                device for device in devices if normalize_mac(device.mac) == hotspot_mac
            ]
            if matches:
                print(f"Устройство найдено по MAC hotspot gateway: {hotspot_mac}")
                return matches[0]
            near_matches = [
                device for device in devices if is_neighbor_mac(device.mac, hotspot_mac)
            ]
            if len(near_matches) == 1:
                print(
                    f"Устройство найдено по соседнему MAC hotspot: hotspot {hotspot_mac}, mDNS {near_matches[0].mac}"
                )
                return near_matches[0]
            if len(near_matches) > 1:
                print(
                    "По соседнему MAC найдено несколько устройств, дополнительно проверяю токен."
                )
                token_matches = [
                    device
                    for device in near_matches
                    if secure_handshake_probe(device, token_hex, udp_timeout)
                ]
                if len(token_matches) == 1:
                    return token_matches[0]
                raise SystemExit(
                    "Не удалось однозначно сопоставить hotspot MAC с mDNS устройством."
                )

        print(
            "MAC не задан, пробую определить нужное устройство по полученному токену..."
        )
        matches = []
...
def build_ha_record(
    device: Any,
    token_hex: str,
    hotspot_handshake_data: dict[str, Any],
    hotspot_mac: str,
) -> dict[str, Any]:
...
def main(argv: Iterable[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Prepare Hommyn UDP Home Assistant settings from pairing mode."
    )
    parser.add_argument(
        "--gateway",
        help="IP шлюза hotspot устройства, по умолчанию default gateway или 192.168.4.1",
    )
    parser.add_argument(
        "--port",
        type=int,
        default=DEFAULT_PORT,
        help="UDP порт устройства, default: 41122",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="Сколько раз запрашивать ненулевой токен",
    )
    parser.add_argument(
        "--discover-timeout", type=float, default=8.0, help="mDNS scan timeout"
    )
    parser.add_argument(
        "--discover-attempts",
        type=int,
        default=6,
        help="Количество mDNS попыток после подключения к домашней сети",
    )
    parser.add_argument(
        "--mac",
        help="MAC устройства, если известен; иначе скрипт попробует найти устройство по токену",
    )
    parser.add_argument(
        "--skip-wifi-question",
        action="store_true",
        help="Не спрашивать про Wi-Fi настройки, только получить токен",
    )
    ...
    if hotspot_mac:
        ...
        print(
            "Не удалось определить MAC hotspot gateway через ARP/ip neigh; продолжу поиск по токену."
        )
